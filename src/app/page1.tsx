  ...
  const pollProcessingStatus = async (statementId: string) => {
    ...
    const poll = async () => {
      try {
        pollCount++;
        console.log(
          `Polling status for statement ${statementId}, attempt ${pollCount}`
        );

        const statusResponse = await fetch(
          `https://fin-trackerr-9c402fb58590.herokuapp.com/v1/statements/${statementId}/status`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        if (statusResponse.ok) {
          ...
          if (
            ...
            const progress = Math.max(timeBasedProgress, apiProgress);
            const remainingTime =
              statusData.estimatedTime ||
              Math.max(Math.ceil((maxDuration - elapsed) / 1000), 10);
            ...
            setProcessingState((prev) => ({
              ...prev,
              progress,
              message: `Processing... (Status: ${apiStatus})`,
            }));
          }
        } else {
          ...
          if (statusResponse.status === 404) {
            throw new Error("Statement not found. Please try uploading again.");
          ...
          setProcessingState((prev) => ({
            ...prev,
            progress,
            message: "Processing... (Status check unavailable)",
          }));
        }
